...
def analyze_permutation_importance(model, X_test, y_test, feature_names):
    ...
    for i, row in perm_importance_df.head(10).iterrows():
        logger.info(f"  {row['feature']}: {row['permutation_importance']:.4f} ± {row['permutation_importance_std']:.4f}")
...
def create_feature_importance_plots(coef_df, perm_df, plots_dir):
    ...
    perm_normalized = perm_df['permutation_importance'] / perm_df['permutation_importance'].max()
    ...
    plt.bar(x - width/2, top_comparison['coefficient_importance'], width, 
            label='Coefficient Importance', alpha=0.8)
    plt.bar(x + width/2, top_comparison['permutation_importance'], width, 
...
def perform_feature_ablation_study(model, X_train, X_test, y_train, y_test, feature_names, top_features=10):
    """
    ...
    for feature in top_feature_names:
        ...
        ablated_model = LogisticRegression(
            C=0.01, penalty='l1', solver='liblinear', max_iter=1000, random_state=RANDOM_STATE
        )
...
def save_feature_analysis_results(coef_df, perm_df, comparison_df, ablation_df, results_dir):
    """
    ...
    results_summary = {
        ...
        'permutation_importance_score': float(perm_df.iloc[0]['permutation_importance']),
...
def main():
    ...
    try:
        ...
        correlation_matrix, high_corr_pairs = analyze_feature_correlations(X_train, feature_names)
        
        # 7. Studio ablation
        ablation_df = perform_feature_ablation_study(model, X_train, X_test, y_train, y_test, feature_names)
        
        # 8. Salvare risultati
        save_feature_analysis_results(coef_df, perm_df, comparison_df, ablation_df, RESULTS_DIR)
        
        # 9. Logging a MLflow
        mlflow_details = log_feature_importance_to_mlflow(coef_df, perm_df, ablation_df, experiment_id)
        ...
        for i, row in perm_df.head(5).iterrows():
            logger.info(f"  {i+1}. {row['feature']}: {row['permutation_importance']:.4f}")
        
        logger.info(f"\n🎯 Most Important Feature: {coef_df.iloc[0]['feature']}")
        logger.info(f"🎯 Coefficient Value: {coef_df.iloc[0]['coefficient']:.4f}")
        logger.info(f"🎯 Permutation Importance: {perm_df.iloc[0]['permutation_importance']:.4f}")
        logger.info(f"{'='*50}")
