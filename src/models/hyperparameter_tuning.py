...
def define_hyperparameter_grids():
    ...
    logger.info(
        f"Grid parameters: {len(grid_params['C']) * len(grid_params['penalty']) * len(grid_params['solver']) * len(grid_params['max_iter']) * len(grid_params['class_weight'])} combinations"
    )
    logger.info(
        f"Random parameters: {len(random_params['C']) * len(random_params['penalty']) * len(random_params['solver']) * len(random_params['max_iter']) * len(random_params['class_weight']) * len(random_params['l1_ratio'])} combinations"
    )
...
def save_tuned_model(model, search_name, results_dir, results):
    ...
    results_path = (
        results_dir / f"logistic_regression_{search_name.lower()}_tuning_results.json"
    )
...
def main():
    ...
    try:
        ...
        random_search = perform_random_search(
            X_train, y_train, random_params, n_iter=50
        )
        ...
        # 6. Logging a MLflow
        logger.info("\n" + "=" * 50)
        grid_mlflow = log_tuning_results_to_mlflow(
            grid_results, grid_model, "GridSearch"
        )
        random_mlflow = log_tuning_results_to_mlflow(
            random_results, random_model, "RandomSearch"
        )
