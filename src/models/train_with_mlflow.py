...
def evaluate_model_with_mlflow(model_config, X_train, X_test, y_train, y_test, model_name):
    """
    ...
    logger.info(f"CV Accuracy: {cv_accuracy.mean():.4f} (+/- {cv_accuracy.std() * 2:.4f})")
...
def create_comparison_report_mlflow(all_results, results_dir):
    ...
    for model_name, results in all_results.items():
        ...
        comparison_data.append({
            ...
            'MLflow_Run_ID': results['model_details'].run_id if results['model_details'] else 'N/A'
        })
...
def main():
    ...
    try:
        ...
        best_model_name = comparison_df.loc[comparison_df['Test_Recall'].idxmax(), 'Model']
