...
def perform_bootstrap_validation(model, X_test, y_test, n_bootstrap=1000):
    ...
    for i in range(n_bootstrap):
        ...
        bootstrap_results["pr_auc"].append(
            average_precision_score(y_boot, y_pred_proba)
        )
    ...
    for metric, values in bootstrap_results.items():
        ...
        logger.info(
            f"  95% CI: [{bootstrap_stats[metric]['ci_95_lower']:.4f}, {bootstrap_stats[metric]['ci_95_upper']:.4f}]"
        )
        logger.info(
            f"  99% CI: [{bootstrap_stats[metric]['ci_99_lower']:.4f}, {bootstrap_stats[metric]['ci_99_upper']:.4f}]"
        )
...
def create_validation_plots(
    ...
    plt.tight_layout()
    plt.savefig(
        plots_dir / "cross_validation_results.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.semilogx(
        param_range, val_mean, "o-", color="red", label="Cross-validation score"
    )
    plt.fill_between(
    ...
    bootstrap_df = pd.DataFrame(
        {
            ...
            "ci_95_lower": [
                bootstrap_stats[m]["ci_95_lower"] for m in bootstrap_stats.keys()
            ],
            "ci_95_upper": [
                bootstrap_stats[m]["ci_95_upper"] for m in bootstrap_stats.keys()
            ],
            "ci_99_lower": [
                bootstrap_stats[m]["ci_99_lower"] for m in bootstrap_stats.keys()
            ],
            "ci_99_upper": [
                bootstrap_stats[m]["ci_99_upper"] for m in bootstrap_stats.keys()
            ],
        }
    )
    ...
    validation_summary = {
        ...
        "learning_curves": {
            ...
            "overfitting_gap": float(
                learning_curves["train_mean"][-1] - learning_curves["val_mean"][-1]
            ),
        },
        "validation_curves": {
            "best_c_value": float(
                validation_curves["param_range"][
                    np.argmax(validation_curves["val_mean"])
                ]
            ),
...
def log_validation_to_mlflow(
    cv_results, bootstrap_stats, learning_curves, validation_curves, experiment_id
):
    """
    ...
    metrics = {
        ...
        "learning_curve_overfitting_gap": learning_curves["train_mean"][-1]
        - learning_curves["val_mean"][-1],
...
def main():
    ...
    try:
        ...
        bootstrap_results, bootstrap_stats = perform_bootstrap_validation(
            model, X_test, y_test
        )
        ...
        # 10. Logging a MLflow
        mlflow_details = log_validation_to_mlflow(
            cv_results,
            bootstrap_stats,
            learning_curves,
            validation_curves,
        ...
        logger.info(
            f"  Recall: {bootstrap_stats['recall']['mean']:.4f} [{bootstrap_stats['recall']['ci_95_lower']:.4f}, {bootstrap_stats['recall']['ci_95_upper']:.4f}]"
        )
        logger.info(
            f"  F1-Score: {bootstrap_stats['f1']['mean']:.4f} [{bootstrap_stats['f1']['ci_95_lower']:.4f}, {bootstrap_stats['f1']['ci_95_upper']:.4f}]"
        )
        ...
        logger.info(
            f"  Overfitting Gap: {learning_curves['train_mean'][-1] - learning_curves['val_mean'][-1]:.4f}"
        )

        logger.info(f"\n🎯 Validation Curves:")
        logger.info(
            f"  Best C Value: {validation_curves['param_range'][np.argmax(validation_curves['val_mean'])]:.4f}"
        )
        ...
        if error_analysis["error_analysis"]:
            logger.info(f"\n❌ Error Analysis:")
            logger.info(
                f"  Total Errors: {error_analysis['error_analysis']['error_count']}"
            )
            logger.info(
                f"  False Positives: {error_analysis['error_analysis']['false_positives']}"
            )
            logger.info(
                f"  False Negatives: {error_analysis['error_analysis']['false_negatives']}"
            )
