...
def feature_engineering(df: pd.DataFrame):
    ...
    mean_features = [col for col in df_processed.columns if 'mean' in col and col != 'diagnosis']
    ...
    if 'radius_mean' in df_processed.columns and 'radius_worst' in df_processed.columns:
        df_processed['radius_ratio'] = df_processed['radius_worst'] / df_processed['radius_mean']
    
    if 'perimeter_mean' in df_processed.columns and 'perimeter_worst' in df_processed.columns:
        df_processed['perimeter_ratio'] = df_processed['perimeter_worst'] / df_processed['perimeter_mean']
    
    if 'area_mean' in df_processed.columns and 'area_worst' in df_processed.columns:
        df_processed['area_ratio'] = df_processed['area_worst'] / df_processed['area_mean']
    
    logger.info(f"Features aggiunte durante feature engineering: {df_processed.shape[1] - df.shape[1]}")
...
def encode_target(df: pd.DataFrame, target_column: str = 'diagnosis'):
    ...
    df_encoded[target_column + '_encoded'] = label_encoder.fit_transform(df_encoded[target_column])
    ...
    target_distribution = df_encoded[target_column + '_encoded'].value_counts().sort_index()
...
def select_features(X: pd.DataFrame, y: pd.Series, k_best: int = 20):
    ...
    logger.info(f"Features selezionate: {len(selected_features)} su {len(numeric_features)}")
...
def scale_features(X_train: np.ndarray, X_test: np.ndarray):
    ...
    logger.info(f"Features scalate - Train: {X_train_scaled.shape}, Test: {X_test_scaled.shape}")
    
    return X_train_scaled, X_test_scaled, scaler

def train_test_split_data(X: np.ndarray, y: pd.Series, test_size: float = TEST_SIZE, random_state: int = RANDOM_STATE):
    """
...
def save_processed_data(X_train_scaled, X_test_scaled, y_train, y_test, 
                       selected_features, scaler, label_encoder, selector):
    """
    ...
    metadata = {
        ...
        'target_mapping': {str(k): int(v) for k, v in zip(label_encoder.classes_, 
